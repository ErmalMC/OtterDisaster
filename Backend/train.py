...
THRESHOLDS = {
    ...
    "cyano_alert": 100.0,  # 10³ cells/ml — WHO bathing ban
...
def load_satellite_data(path: Path) -> pd.DataFrame:
    ...
    Expected columns: timestamp, pixel_id (or lat/lon), ndwi, ndti, ndci,
                      chl_a, cyanobacteria, turbidity_sat, cdom, doc
    """
    ...
    # Filter non-water pixels
    n_before = len(df)
    df = df[df["ndwi"] >= THRESHOLDS["ndwi_min"]]
    log.info(f"Water mask: kept {len(df)}/{n_before} observations (NDWI >= {THRESHOLDS['ndwi_min']})")
...
def load_sensor_data(path: Path) -> pd.DataFrame:
    ...
    df = df.set_index("timestamp").resample("D").median().reset_index()
